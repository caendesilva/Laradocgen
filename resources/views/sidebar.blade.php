        ...
        <div class="md:flex md:flex-col md:items-stretch md:opacity-100 md:relative md:shadow-none shadow absolute top-0 left-0 right-0 z-40 overflow-y-auto overflow-x-hidden h-auto items-center flex-1 rounded hidden" id="example-collapse-sidebar">
            <div class="md:min-w-full md:hidden block pb-4 mb-4 border-b border-solid border-zinc-200">
                <div class="flex flex-wrap">
                    <div class="w-9/12"> <a class="md:block text-left md:pb-2 text-zinc-600 dark:text-zinc-300 mr-0 inline-block whitespace-nowrap text-sm uppercase font-bold p-4 px-0" href="index{{ $realtime == false ? '.html' : '' }}"> {{ $siteName }} </a> </div>
            ...
            <ul class="md:flex-col md:min-w-full flex flex-col list-none">
                ...
                <li id="sidebar-active" class="items-center"> <a class="text-pink-500 hover:text-pink-600 text-xs uppercase py-3 font-bold block" href="{{ $link->slug }}{{ $realtime == false ? '.html' : '' }}"> {{ $link->title }} </a> </li>
                @else 
                <li class="items-center"> <a class="text-zinc-700 dark:text-gray-200 hover:text-zinc-500 text-xs uppercase py-3 font-bold block" href="{{ $link->slug }}{{ $realtime == false ? '.html' : '' }}"> {{ $link->title }} </a> </li>
                @endif @endforeach 
            </ul>
            <div class="mt-auto">
